...
data_normalised <- data.deletion %>%
  ...
  mutate(education = as.factor(education)) %>% # change education to factor
  mutate(TenYearCHD = as.factor(TenYearCHD)) %>% # change TenYearCHD to factor
...
```{r}
mod1 <- glm(TenYearCHD ~ ., data = data_normalised, family = "binomial")
summary(mod1)
```
...
test_confusion_matrix <- confusionMatrix(test_predictions, test_data$TenYearCHD)
...
```
```{r}
rf_test  <- predict(model_rf, newdata = test_data)
