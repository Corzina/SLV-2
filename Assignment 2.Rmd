...
data_standardized <- na.omit(data_normalised) %>%
  mutate_if(is.numeric, standardize)
data_standardized$TenYearCHD <- as.factor(data.deletion$TenYearCHD)
...
```{r}
data_standardized %>%
  pivot_longer(cols = -c(TenYearCHD), names_to = "name", values_to = "value") %>%
...
```{r}
mod1 <- glm(TenYearCHD ~ male + age + currentSmoker + diabetes + BMI + glucose, data = data_standardized, family = "binomial")
summary(mod1)
```
...
train_predictions_rf <- predict(model_rf, newdata = train_predictors)
test_predictions_rf <- predict(model_rf, newdata = test_predictors)

# Confusion matrix random forest
train_confusion_matrix_rf <- confusionMatrix(train_predictions_rf, train_target)
test_confusion_matrix_rf <- confusionMatrix(test_predictions_rf, test_target)

# Print the confusion matrix for the random forest model on the test and train data
print(test_confusion_matrix_rf)
print(train_confusion_matrix_rf)
```
Accuracy: The overall accuracy is 84.52%, meaning the model correctly predicted the outcome for 84.52% of the test set.
True Negatives (TN): 711 patients were correctly predicted as not having heart disease.
False Positives (FP): 7 patients were incorrectly predicted as having heart disease.
True Positives (TP): 4 patients were correctly predicted as having heart disease.
False Negatives (FN): 124 patients were incorrectly predicted as not having heart disease.
...
train_y <- as.numeric(train_target) - 1
...
table(pred$Disease, train_data$TenYearCHD)
...
```
```{r}
rf_test  <- predict(model_rf, newdata = test_predictors)
...
list(rf_test, 
     xgb_test) %>%
  map(~ confusionMatrix(.x, test_data$TenYearCHD))
```
